...
public class InteractiveTicketUpdater extends InteractiveObjectUpdater<Ticket> {
    private <T extends IntInputDevice & EnumInputDevice> void init(InteractiveUpdater<Coordinates> coordUpdate, InteractiveUpdater<Event> eventUpdate, T in) {
      addInteractiveChange(Ticket::setName, Ticket::getName, String.class, new UserInteractor<String>("Название билета", in::read, "[ERROR] Неправильный формат ввода: название не должно быть пустым."), TicketValidator::validateName);
      addInteractiveUpdater(coordUpdate, Ticket::setCoordinates, Ticket::getCoordinates, Coordinates.class);
      addInteractiveChange(Ticket::setPrice, Ticket::getPrice, Integer.class, new UserInteractor<Integer>("Стоимость билета", () -> { Optional<Integer> res = in.readInt(); in.skipLine(); return res; }, "[ERROR] Неправильный формат ввода: введите натуральное число.", "в у.е."), TicketValidator::validatePrice);
      ...
      addInteractiveChange(Ticket::setType, Ticket::getType, TicketType.class, new UserInteractor<Enum<TicketType>>("Тип билета", () -> { Optional<Enum<TicketType>> res = in.readEnum(TicketType.class); in.skipLine(); return res; }, "[ERROR] Неправильный формат ввода: указанный тип билета не найден.", List.of(options)), TicketValidator::validateType);
   ...
   public <T extends IntInputDevice & EnumInputDevice> InteractiveTicketUpdater(InteractiveUpdater<Coordinates> coordUpdate, InteractiveUpdater<Event> eventUpdate, T in, OutputDevice out, List<Interactor<?>> interactors, List<TypedBiConsumer<Ticket,?>> setters, List<Object> objects, List<Supplier<?>> methods, List<TypedPredicate<?>> validators, List<Function<Ticket,?>> getters, List<InteractiveUpdater> updaters) {
