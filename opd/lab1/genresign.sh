...
# generating man page
man <(cat pattern.txt | sed -e "{s/SURNAME/$SURNAME/}" -e "{s/NAME/$NAME/}" -e "{s/PATRONYMIC/$PATRONYMIC/}" -e "{s/GROUP/$GROUP/}" -e "{s/FACULTY/$FACULTY/}" -e "{s/PHONE/${PHONE:--}/}" -e "{s/EMAIL/${EMAIL:--}/}" -e "s/DATE/$(date +"%d.%m.%Y")/")
