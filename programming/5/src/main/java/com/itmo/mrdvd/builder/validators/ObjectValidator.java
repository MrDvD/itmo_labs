...
public class ObjectValidator<T> implements Validator<T> {
   ...
   public ObjectValidator(List<Function<T,?>> getters, List<TypedPredicate<?>> methods, List<Validator> validators) {
   ...
   @Override
   public <U> ObjectValidator<T> check(Function<T,U> getter, Class<U> valueCls, Predicate<U> validator) throws IllegalArgumentException {
      if (getter == null) {
   ...
   @Override
   public <U> Validator<T> check(Function<T,U> getter, Validator<U> validator) throws IllegalArgumentException {
      if (getter == null) {
         throw new IllegalArgumentException("Getter не может быть null.");
      }
      if (validator == null) {
   ...
   protected ProcessStatus processValidator(int index) {
      return validators.get(index).validate(getters.get(index).apply(object)) ? ProcessStatus.SUCCESS : ProcessStatus.FAILURE;
   }
